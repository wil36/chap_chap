          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                StreamBuilder(
                  stream: getRecipeLikes(streamPremium.reference!),
                  builder: (context, snapshot) {
                   if (snapshot.hasData) {
      List<DocumentSnapshot> documents = snapshot.data!.docs;
      for (DocumentSnapshot doc in documents) {
        List<dynamic> tableauDeMaps = doc['nomDuTableau'];
        for (Map<dynamic, dynamic> map in tableauDeMaps) {
          if (map.containsKey('ID') && map['ID'] == votreID) {
            // ID trouvé dans le tableau de maps
            return Text('ID trouvé dans le document ${doc.id}');
          }
        }
      }
      // ID non trouvé dans aucun document
      return Text('ID non trouvé');
...
Dans cet exemple, nous avons utilisé la méthode ".snapshots()" pour créer un flux de documents Firebase en temps réel pour une collection donnée. Ensuite, nous avons utilisé le widget "StreamBuilder" pour écouter ce flux de données et construire une interface utilisateur en conséquence.
  ...
  getRecipeLikes(DocumentReference<Object?> documentReference) {
    print(documentReference);
    return FirebaseFirestore.instance.collection('likes').snapshots();
  }
}
