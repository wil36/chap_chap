                ...
                child: Container(
                  ...
                  child: ToggleIcon(
                    onPressed: () async {
                      ...
                      await currentUserDocument!.reference!.update(usersUpdateData);
                      print(favorisRecettesElement!.id);
                      final currentRecipe = await FirebaseFirestore.instance.collection('recettes').doc(favorisRecettesElement!.id).get();
                      List<String> currentRecipeLikes = currentRecipe.data()!['favoris_user_id'];
                      currentRecipeLikes == null ? currentRecipeLikes = [] : currentRecipeLikes = currentRecipeLikes;
                      currentRecipeLikes.contains(currentUserReference!.id) ? currentRecipeLikes.remove(currentUserReference!.id) : currentRecipeLikes.add(currentUserReference!.id);
                      await FirebaseFirestore.instance.collection('recettes').doc(favorisRecettesElement.id).update({
                        'favoris_user_id': currentRecipeLikes,
                      });
                    },
                    value: currentUserDocument!.favorisRecettes!.toList().contains(streamPremium.reference),
                    onIcon: SvgPicture.asset(
