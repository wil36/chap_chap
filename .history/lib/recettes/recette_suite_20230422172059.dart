                ...
                StreamBuilder(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      ...
                      final Map<String, dynamic> document = snapshot.data!.docs.fold(
                        {},
                        (previousValue, doc) => {...previousValue, ...doc.data()},
                      );

                      List<dynamic> favRecipe = document['favorisRecettes'];
                      print(favRecipe);
                      if (favRecipe.contains(streamPremium.reference!)) {
                        likesCounter++;
                        if (usersInitials.length < 3 && document.containsKey('Nom') && document.containsKey('Prenom')) {
                          String firstNameInitials = document['Prenom'].toString().toUpperCase()[0];
                          String lastNameInitials = document['Nom'].toString().toUpperCase()[0];
