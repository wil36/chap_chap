...
class _RecetteSuiteWidgetState extends State<RecetteSuiteWidget> {
                      ...
                      for (DocumentSnapshot doc in documents) {
                        ...
                        if (favRecipe.contains(streamPremium.reference!)) {
                          ...
                          if (usersInitials.length < 3) {
                            String firstNameInitials = doc['display_name'].toString().toUpperCase() ?? 'L';
                            String lastNameInitials = doc['display_name'].toString().toUpperCase() ?? 'B';
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getUsers() {
    return FirebaseFirestore.instance.collection('users').where('favorisRecettes', isGreaterThan: []).where('Nom', isGreaterThan: '').where('Prenom', isGreaterThan: '').snapshots();
  }
}
