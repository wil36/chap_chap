...
class _RecetteSuiteWidgetState extends State<RecetteSuiteWidget> {
                    ...
                    if (snapshot.hasData) {
                      ...
                      for (DocumentSnapshot doc in documents) {
                        Future.delayed(Duration.zero, () async {
                          await getPictureUrl(doc['user_id'].toString().split('/')[1]);
                        });
                        List<dynamic> recettesIds = doc['recette_id'];
                        for (Map<dynamic, dynamic> map in recettesIds) {
                          if (map.containsKey('ref') && map['ref'] == streamPremium.reference) {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> getRecipeLikes(DocumentReference<Object?> documentReference) {
    return FirebaseFirestore.instance.collection('likes').snapshots();
  }

  //get picture url via user id
  Future<void> getPictureUrl(String userId) async {
    print(userId);
    // String url = '';
    // await FirebaseFirestore.instance.collection('users').doc(userId).get().then((value) {
    //   if (value.data()!.containsKey('photo_url')) {
