...
Future resetPassword({required String email, BuildContext? context}) async {
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text(
              'Impossible de vous envoyer votre mot de passe sur votre adresse email')),
    );
    return null;
  }
  ScaffoldMessenger.of(context!).showSnackBar(
    const SnackBar(
        content: Text(
            'Une demande de réinitialisation de mot de passe vient d’être envoyée sur ton adresse e-mail')),
  );
}
  ...
  await FirebaseAuth.instance.verifyPhoneNumber(
    ...
    verificationFailed: (exception) {
      ScaffoldMessenger.of(context!).showSnackBar(const SnackBar(
        content: Text(
            'Oups, il y a une erreur dans la saisie de votre numéro de téléphone'),
      ));
    },
