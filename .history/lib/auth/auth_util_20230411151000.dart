...
Future<User?> signInOrCreateAccount(BuildContext? context, Future<UserCredential> Function() signInFunc) async {
  try {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Oups, vérifiez votre saisie...')),
    );
    return null;
  }
}
...
Future resetPassword({required String email, BuildContext? context}) async {
  try {
    ...
    ScaffoldMessenger.of(context!).hideCurrentSnackBar();
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Impossible de vous envoyer votre mot de passe sur votre adresse email')),
    );
    return null;
  }
  ScaffoldMessenger.of(context!).showSnackBar(
    const SnackBar(content: Text('Une demande de réinitialisation de mot de passe vient d’être envoyée sur ton adresse e-mail')),
  );
}
...
String get currentUserUid => currentUserDocument?.uid ?? currentUser?.user?.uid ?? '';

String get currentUserDisplayName => currentUserDocument?.displayName ?? currentUser?.user?.displayName ?? '';

String get currentUserBio => currentUserDocument?.bio ?? currentUser?.user?.displayName ?? '';

String get currentUserTikTok => currentUserDocument?.tiktok ?? currentUser?.user?.tiktok ?? '';

String get currentUserInstagram => currentUserDocument?.instagram ?? currentUser?.user?.instagram ?? '';
  ...
  await FirebaseAuth.instance.verifyPhoneNumber(
    ...
    verificationFailed: (exception) {
      ScaffoldMessenger.of(context!).showSnackBar(const SnackBar(
        content: Text('Oups, il y a une erreur dans la saisie de votre numéro de téléphone'),
      ));
    },
